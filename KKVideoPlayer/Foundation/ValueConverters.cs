...
namespace KKVideoPlayer.Foundation
{
    ...
    internal class SecondsToTimeSpanConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            {
                double double_time = double.Parse((string)value);
                var result = TimeSpan.FromTicks(System.Convert.ToInt64(10000 * double_time));

                string output = null;
                output = string.Format("{0:hh}:{0:mm}:{0:ss}.{0:ff}", result);

                // TimeSpan.ParseExact(output, @"mm\:ss\.ff", result);
                return output;
            }
            else
            {
